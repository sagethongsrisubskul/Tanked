	...
	public static void powerstatus()
		...
		if(Settings.playerType == C.SERVER)
			...
			*/
			...
			int ycoord = 0;
			int index = 0;
			if((StatePlay.seconds == Settings.powerupInterval) && StatePlay.powerupflag == false)
				{
				...
				index = ThreadLocalRandom.current().nextInt(0, Filenames.powerupIcons.length);
				//send to clients
				NetworkControl.sendToAll("~PT" + xcoord + "," + ycoord + "," + index);
				//powerupflag=true;
				}
			if((StatePlay.seconds == Settings.powerupInterval + Settings.powerupDuration) && StatePlay.powerupflag == true)
				{
