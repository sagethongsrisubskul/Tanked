...
// Try first to get it from the actual path specified...
		...
		if(u == null)
			...
			for(StackTraceElement ste : new Throwable().getStackTrace())
				...
				if(!ste.getClassName().startsWith("jig") && !ste.getClassName().startsWith("org.newdawn.slick"))
					...
					try
						{
						System.err.println("  - searching from location of class: " + ste.getClassName());
					...
					if(u != null)
						{
						System.err.println(" - Found resource '" + rscName + "' by inferring path. This is fragile;" + " use a fully qualified path for your release.");
						return u;
						}
					}
				}
	 ...
	 * calls for it.
	 *
	 * @param rscName The name/pathspec of the resource to load
	 ...
	 * quite a bit of space. Calling this should helpInfo with that.
	 */
	 ...
	 * this should helpInfo with that.
	 */
